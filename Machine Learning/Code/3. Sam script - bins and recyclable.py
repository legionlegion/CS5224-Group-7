...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Append blue-bin YOLO bbox labels on top of existing label files."
    )
    parser.add_argument(
        "--input-source",
...
def yolo_to_xyxy(x, y, w, h, image_w, image_h):
...
def find_existing_recyclable_line(label_lines):
    for ln in label_lines:
        parsed = parse_yolo_line(ln)
        if parsed is None:
            continue
        class_id = parsed[0]
        if class_id != CLASS_NAME_TO_ID["blue bins"]:
            return parsed
    return None


def draw_two_item_overlay(image_path, recyclable_entry, blue_entry, output_overlay_path):
    ...
    # Make annotations very obvious.
    box_thickness = 10
    text_scale = 2.0
    text_thickness = 6

    rec_class_id, rec_x, rec_y, rec_w, rec_h = recyclable_entry
    rec_x1, rec_y1, rec_x2, rec_y2 = yolo_to_xyxy(rec_x, rec_y, rec_w, rec_h, w, h)
    rec_color = (0, 255, 255)
    rec_text = f"recyclable: {id_to_name.get(rec_class_id, str(rec_class_id))}"
    cv2.rectangle(img, (rec_x1, rec_y1), (rec_x2, rec_y2), rec_color, box_thickness)
    cv2.putText(
        img,
        rec_text,
        (max(5, rec_x1), max(70, rec_y1 - 20)),
    ...
    blue_class_id, blue_x, blue_y, blue_w, blue_h = blue_entry
    blue_x1, blue_y1, blue_x2, blue_y2 = yolo_to_xyxy(blue_x, blue_y, blue_w, blue_h, w, h)
    blue_color = (255, 140, 0)
    blue_text = "blue bins"
    cv2.rectangle(img, (blue_x1, blue_y1), (blue_x2, blue_y2), blue_color, box_thickness)
    cv2.putText(
        img,
        blue_text,
        (max(5, blue_x1), max(140, blue_y1 - 20)),
        cv2.FONT_HERSHEY_SIMPLEX,
        text_scale,
        blue_color,
        text_thickness,
    ...
    cv2.imwrite(output_overlay_path, img)
...
def mask_to_yolo_line_from_image(image_path, mask, class_id):
    original_bgr = cv2.imread(image_path)
    if original_bgr is None:
        raise ValueError(f"Could not load image: {image_path}")
    ...
    return bbox_to_yolo_line(bbox, w, h, class_id)
...
def main():
    ...
    blue_bin_class_id = CLASS_NAME_TO_ID["blue bins"]

    summary_rows = []
    print(f"Starting blue-bin add-on for {len(image_paths)} image(s).")
    ...
    for image_path in image_paths:
        ...
        try:
            if not os.path.exists(yolo_label_path):
                print(f"Skipping {filename}: existing label not found at {yolo_label_path}")
                summary_rows.append([filename, 0, "", "label_missing", "", ""])
                continue

            with open(yolo_label_path, "r", encoding="utf-8") as f:
                existing_lines = [ln.strip() for ln in f.readlines() if ln.strip()]

            recyclable_entry = find_existing_recyclable_line(existing_lines)
            if recyclable_entry is None:
                print(f"Skipping {filename}: no existing recyclable line found in {yolo_label_path}")
                summary_rows.append([filename, 0, "", "recyclable_missing", "", ""])
                continue

            blue_mask, blue_score, elapsed_ms = detect_single_best(image_path, "blue bins", processor)
            if blue_mask is None:
                print(f"No blue-bin detection: {filename} | Time: {elapsed_ms:.2f} ms")
                summary_rows.append([filename, 0, "", "no_blue_detection", "", f"{elapsed_ms:.2f}"])
                continue

            blue_yolo_line = mask_to_yolo_line_from_image(image_path, blue_mask, blue_bin_class_id)
            if not blue_yolo_line:
                print(f"Skipping {filename}: blue-bin mask produced empty bbox")
                summary_rows.append([filename, 0, f"{blue_score:.6f}", "empty_blue_bbox", "", f"{elapsed_ms:.2f}"])
                continue

            if blue_yolo_line in existing_lines:
                print(f"No append (already exists): {filename} | Blue-bin score: {blue_score:.4f} | Time: {elapsed_ms:.2f} ms")
                summary_rows.append([filename, 0, f"{blue_score:.6f}", "duplicate_blue_line", "blue bins", f"{elapsed_ms:.2f}"])
            else:
                with open(yolo_label_path, "a", encoding="utf-8") as f:
                    f.write(blue_yolo_line + "\n")

            blue_entry = parse_yolo_line(blue_yolo_line)
            if blue_entry is None:
                print(f"Skipping overlay for {filename}: failed to parse blue-bin line")
                summary_rows.append([filename, 0, f"{blue_score:.6f}", "blue_parse_failed", "", f"{elapsed_ms:.2f}"])
                continue

            overlay_path = os.path.join(overlay_folder, f"{base_name}_overlay.jpg")
            draw_two_item_overlay(image_path, recyclable_entry, blue_entry, overlay_path)

            print(f"Appended blue-bin label: {filename} | Blue-bin score: {blue_score:.4f} | Time: {elapsed_ms:.2f} ms")
            summary_rows.append([filename, 1, f"{blue_score:.6f}", "blue bins", "blue bins", f"{elapsed_ms:.2f}"])
        except Exception as e:
            print(f"Skipping {filename}: {e}")
            summary_rows.append([filename, 0, "", "error", "", ""])

    summary_path = os.path.join(output_base_folder, "detection_summary.csv")
    with open(summary_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["image_name", "blue_bin_added", "blue_score", "status", "class_name", "inference_time_ms"])
